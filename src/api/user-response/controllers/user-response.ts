...
module.exports = createCoreController('api::user-response.user-response', ({ strapi }) => ({
  async create(ctx) {
    const { survey, content } = ctx.request.body.data;

    // Check for required data
    if (!Array.isArray(content) || content.length === 0 || !survey) {
   ...
   // Check each answer belongs to a question from this survey (by documentId)
    const answerQuestionDocIds = content.map(aid => answerToQuestionDocIdMap.get(aid));
